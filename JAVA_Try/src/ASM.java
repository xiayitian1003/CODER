...
import com.transsion.deskclock.AlarmUtils;
import com.transsion.deskclock.AsyncHandler;
...
import com.transsion.deskclock.provider.Alarm;
import com.transsion.deskclock.provider.AlarmInstance;
import com.transsion.deskclock.settings.SettingsActivity;
...
import static com.transsion.deskclock.alarms.AlarmShutDownManager.SUPPORT_SPRD_SHUTDOWN_ALARM;
...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    private static void updateNextAlarm(Context context) {
        ...
        //MTK,SPRD Compatible adapter by yongwen.huang 2020/2/27
        if(Utils.isMediaTekPlatform()){
            setPoweroffAlarm(context, nextAlarm);
        } else if(SUPPORT_SPRD_SHUTDOWN_ALARM){
            /*itel begin, adapt power off_bubiao.xie_2018-08-31*/
     ...
     * <uses-permission android:name="android.permission.WRITE_SETTINGS" /> deprecation
     */
    ...
    private static void updateNextAlarmInAlarmManager(Context context, AlarmInstance nextAlarm) {
        ...
        final int flags = nextAlarm == null ? (PendingIntent.FLAG_IMMUTABLE|PendingIntent.FLAG_NO_CREATE) : (PendingIntent.FLAG_IMMUTABLE);
    ...
    /*Transsion begin, add Game mode_bubiao.xie_2020-03-25*/
    public static Intent createStateChangeIntent(Context context, String tag,
        ...
        // occur. To be safer, the call begins in AlarmService, which has the power to display the
        // firing alarm if needed, so no jump is needed.
        Intent intent = AlarmInstance.createIntent(context, AlarmService.class, id);
        intent.setAction(CHANGE_STATE_ACTION);
        intent.addCategory(tag);
        intent.putExtra(ALARM_GLOBAL_ID_EXTRA, DataModel.getDataModel().getGlobalIntentId());
        if (state != null) {
    ...
    public static void setLowNotificationState(Context context, AlarmInstance instance) {
        ...
        AlarmInstance.updateInstance(contentResolver, instance);
        /*itel begin, adapt sprd power on off_bubiao.xie_2018-09-06, end*/
        AlarmShutDownManager.createOrUpdateShutDownFile(context, instance);
    ...
    public static void setHideNotificationState(Context context, AlarmInstance instance) {
        ...
        AlarmInstance.updateInstance(contentResolver, instance);
        /*itel begin, adapt sprd power on off_bubiao.xie_2018-09-06*/
        AlarmShutDownManager.createOrUpdateShutDownFile(context, instance);
     ...
     * the application notifications and schedule any state changes that need
     * to occur in the future.
     *Setting upcoming AlarmClockInfo for alarm
     * @param context  application context
     * @param instance to set state to
     */
    ...
    public static void setSnoozeState(final Context context, AlarmInstance instance,
            boolean showToast) {
        LogUtils.d("setSnoozeState instance" + instance);
        // Stop alarm if this instance is firing it
        AlarmService.stopAlarm(context, instance);

        // Calculate the new snooze alarm time
        int snoozeMinutes = DataModel.getDataModel().getSnoozeLength();
        if (instance.snoozeDuration > 0){
            snoozeMinutes = instance.snoozeDuration;
        }
        ...
        if (showToast) {
            ...
            int finalSnoozeMinutes = snoozeMinutes;
            final Runnable myRunnable = new Runnable() {
                @Override
                public void run() {
                    String displayTime = String.format(context.getResources().getQuantityText
                            (R.plurals.alarm_alert_snooze_set, finalSnoozeMinutes).toString(),
                            finalSnoozeMinutes);
    ...
    public static void setPreDismissState(Context context, AlarmInstance instance) {
        ...
        /*itel begin, adapt sprd power on off_bubiao.xie_2018-09-06*/
        // when click "Dismiss now" or dismiss alarm in alarm notification, poweron off alarm can still work
        if(SUPPORT_SPRD_SHUTDOWN_ALARM){
    ...
    public static void handleIntent(Context context, Intent intent) {
        ...
        if (CHANGE_STATE_ACTION.equals(action)) {
            ...
            /*Transsion begin, add MUSLIMS clock function_bubiao.xie_2020-02-29*/
            Intent viewAlarmIntent = Alarm.createIntent(context, DeskClock.class, alarmId).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            if(null != instance && instance.mIsMuslimsAlarm) {
                intent.putExtra(MuslimAlarmFragment.SWITCH_TO_MUSLIMS_ALARM_INTENT_EXTRA, true);
            } else {
                intent.putExtra(AlarmClockFragment.SCROLL_TO_ALARM_INTENT_EXTRA, alarmId);
            }
            /*Transsion end*/
    ...
    private static class AlarmManagerStateChangeScheduler implements StateChangeScheduler {
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time,
            ...
            PendingIntent pendingIntent = PendingIntent.getService(context, instance.hashCode(),
                    stateChangeIntent, PendingIntent.FLAG_IMMUTABLE|PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            if (Utils.isMOrLater()) {
                ...
                try {
                    ...
                    LogUtils.v("Maximum limit of concurrent alarms 500 reached");
                }
            }else{
            ...
            if(Utils.isMediaTekPlatform()){
                if(instance.mAlarmState == AlarmInstance.PREDISMISSED_STATE){
                    Utils.cancelPowerOffAlarm(am, context.getPackageName());
                }
            }else if(SUPPORT_SPRD_SHUTDOWN_ALARM){
                /*itel begin, adapt sprd power on off_bubiao.xie_2018-09-06*/
        ...
        public void cancelScheduledInstanceStateChange(Context context, AlarmInstance instance) {
            ...
            if (pendingIntent != null) {
                ...
                if(Utils.isMediaTekPlatform()){
                    ...
                    ///@}*/
                    /* add poweronoff alarm @{
                     * @orig
                     * am.cancel(pendingIntent);
                     * */
//                    AlarmManagerWrapper.cancelAlarm(am, pendingIntent, null);
                    Utils.cancelAlarm(am, pendingIntent);
    ...
    public static void updatePowerOffAlarm(Context context, AlarmInstance instance) {
        ...
        for (AlarmInstance alarmInstance : alarmInstances) {
            if (alarmInstance!= null && alarmInstance.getAlarmTime().compareTo(instance.getAlarmTime()) != 0) {
                ...
                AlarmShutDownManager.createOrUpdateShutDownFile(context, alarmInstance, am, alarmInstance.mAlarmState, true);
            }
        }
    }
    ...
    public static void setPoweroffAlarm(Context context, AlarmInstance nextAlarm) {
        if (nextAlarm != null && PowerOffAlarm.canEnablePowerOffAlarm()) {
            ...
            LogUtils.i("Set for PowerOffAlarm alarmType = 8, time = "
    ...
    public static void resetServiceForAlarm(Context context, AlarmInstance nextAlarm) {
        ...
        PendingIntent pendingIntentAlarm = PendingIntent.getService(context, nextAlarm.hashCode(),
                stateChangeIntentAlarm, PendingIntent.FLAG_IMMUTABLE|PendingIntent.FLAG_UPDATE_CURRENT);
